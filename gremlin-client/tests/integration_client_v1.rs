...
fn test_client_connection_ok_v1() {
    expect_client_serializer(GraphSON::V1);
}
...
fn test_wrong_query_v1() {
    let error = graph_serializer(GraphSON::V1)
        .execute("g.V", &[])
...
fn test_vertex_query_v1() {
    let graph = graph_serializer(GraphSON::V1);
    let vertices = graph
        .execute(
...
#[test]
fn test_complex_vertex_creation_with_properties_v1() {
